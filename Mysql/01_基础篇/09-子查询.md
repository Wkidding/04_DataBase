...
```mysql
[root 3306][DB: atguigudb] mysql >SELECT last_name, job_id
    -> FROM   employees
    -> WHERE  job_id =
    ->                 (SELECT job_id
    ->                  FROM   employees
...
```sql
[root 3306][DB: atguigudb] mysql >SELECT employee_id, last_name
    -> FROM   employees
    -> WHERE  salary =
    ->                 (SELECT   MIN(salary)
    ->                  FROM     employees
...
```mysql
[root 3306][DB: atguigudb] mysql >SELECT e.employee_id, last_name,e.job_id FROM employees e where 2<=(select count(*) from job_history j where e.employee_id=j.employee_id);
+-------------+-----------+---------+
...
```sql
[root 3306][DB: atguigudb] mysql >SELECT employee_id, last_name, job_id, department_id FROM employees e1 WHERE  EXISTS ( SELECT * FROM employees e2 WHERE e2.manager_id = e1.employee_id);
+-------------+-----------+---------+---------------+
...
```sql
[root 3306][DB: atguigudb] mysql >select employee_id, last_name, job_id, department_id from employees where employee_id in (select distinct manager_id from employees);
+-------------+-----------+---------+---------------+
| employee_id | last_name | job_id  | department_id |
+-------------+-----------+---------+---------------+
|         100 | King      | AD_PRES |            90 |
...
```sql
## 方式1：使用join
[root 3306][DB: atguigudb] mysql >SELECT d.department_id, d.department_name FROM employees e RIGHT JOIN departments d on e.department_id = d.department_id WHERE e.department_id is null;

## 方式2：使用NOT EXISTS
[root 3306][DB: atguigudb] mysql >SELECT department_id, department_name FROM departments d WHERE NOT EXISTS (SELECT 'X' FROM employees WHERE department_id = d.department_id);
+---------------+----------------------+
