...
可以对**任意数据类型**的数据使用 MIN 和 MAX 函数。 ---- **==适用于数值、字符串、日期==**
...
```sql
## count后面跟列明时，该列不能含有null值
[root 3306][DB: atguigudb] mysql >SELECT COUNT(commission_pct),sum(commission_pct)/count(commission_pct),sum(commission_pct)/107
    -> FROM   employees
    -> WHERE  department_id = 50;
+-----------------------+-------------------------------------------+-------------------------+
| COUNT(commission_pct) | sum(commission_pct)/count(commission_pct) | sum(commission_pct)/107 |
+-----------------------+-------------------------------------------+-------------------------+
|                     0 |                                      NULL |                    NULL |
+-----------------------+-------------------------------------------+-------------------------+
1 row in set (0.00 sec)
...
```sql
[root 3306][DB: atguigudb] mysql >SELECT   department_id, AVG(salary)
    -> FROM     employees
...
```sql
[root 3306][DB: atguigudb] mysql >SELECT AVG(salary)
    -> FROM     employees
    -> GROUP BY department_id ;
+--------------+
| AVG(salary)  |
...
-- group by位置：①from后 ②where后 ③order by后 ④limit前
```
...
-- where & having区别（下文解释）
-- 方式1，执行效率高（推荐使用）
[root 3306][DB: atguigudb] mysql>select department_id, max(salary) from employees where department_id in (10,20,30) group by department_id having MAX(salary) > 10000;
+---------------+-------------+
| department_id | max(salary) |
+---------------+-------------+
|            20 |    13000.00 |
...
SELECT ..., ...., ...
FROM   ..., ..., ....
WHERE 多表的连接条件 AND 不包含组/聚合函数的过滤条件
GROUP BY ..., ...
HAVING 包含组/聚合函数的过滤条件
ORDER BY ... ASC/DESC
LIMIT ..., ...
...
ON 多表的连接条件
JOIN ... ON ...
WHERE 不包含组/聚合函数的过滤条件
AND/OR 不包含组/聚合函数的过滤条件
GROUP BY ..., ...
HAVING 包含组/聚合函数的过滤条件
ORDER BY ... ASC/DESC
LIMIT ..., ...
