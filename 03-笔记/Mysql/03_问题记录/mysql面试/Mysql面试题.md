...
事务是数据库的逻辑工作单位，事务中包含的各操作**要么都做，要么都不做**。主要==依靠 **Undo Log**==。事务执行中记录Undo，若事务回滚，则根据Undo Log将数据恢复到修改前的状态。

#### **2 、一致性**  (Consistency)

事务执行的结果必须是使数据库从一个一致性状态变到另一个一致性状态。因此当数据库只包含成功事务提交的结果时，就说数据库处于一致性状态。如果数据库系统 运行中发生故障，有些事务尚未完成就被迫中断，这些未完成事务对数据库所做的修改有一部分已写入物理数据库，这时数据库就处于一种不正确的状态，或者说是 不一致的状态。

 这是数据库的最终目标，==依赖原子性、隔离性、持久性共同保证==。同时还包括约束（主键、外键、唯一索引、Check约束）和触发器来维护业务逻辑一致性。

#### **3 、隔离性**  (Isolation)

一个事务的执行不能其它事务干扰。即一个事务内部的操作及使用的数据对其它并发事务是隔离的，并发执行的各个事务之间不能互相干扰。 

依靠 **锁** + **MVCC**。**==读操作==**主要利用MVCC（通过ReadView和隐藏列）实现非阻塞读；**==写操作==**和“当前读”需要加锁（Record Lock、Gap Lock、Next-Key Lock）防止幻读。

#### **4 、持续性**/**永久性** (Durability)

也称永久性，指一个事务一旦提交，它对数据库中的数据的改变就应该是永久性的。接下来的其它操作或故障不应该对其执行结果有任何影响。

==依靠 **Redo Log**==。当Commit时，先将日志写入Redo Log Buffer并刷盘（innodb_flush_log_at_trx_commit控制），即使MySQL宕机，重启后也能通过Redo Log重做，防止数据丢失。



### 063 并发事务会有哪些问题？

多个事务并发执行一定会产生相互争夺资源的问题。会带来脏读，不可重复读，幻读等问题。

-   **脏读**：读到了其他事务**未提交**的数据。若那个事务回滚，当前事务读到的数据就是无效的。
    -   *例*：事务A修改工资为10k未提交，事务B读到10k，A回滚变为5k，B读到的10k就是脏数据。
-   **不可重复读**：在同一事务内两次读取**同一行数据**，结果不一致（重点：Update/Delete）。
    -   *例*：事务A第一次读到工资5k，事务B将工资改为10k并提交，事务A第二次读到10k。
-   **幻读**：在同一事务内两次执行同样的范围查询，第二次返回了第一次未出现的行（重点：Insert）。
    -   *例*：事务A查询部门有3人，事务B向该部门插入1人并提交，事务A再查发现变成4人，多了“幻影”行。

**区分关键**：不可重复读侧重于“值被改了”，幻读侧重于“记录条数变了”。

| 问题                               | 数据是否正确                       | 是否需事务隔离 |
| :--------------------------------- | :--------------------------------- | :------------- |
| **脏读**（Dirty read）             | 严重错误（数据从未真正存在）       | 必须避免       |
| **不可重复读**（Unrepeatableread） | 数据虽真实，但前后矛盾（值被改了） | 视业务需求而定 |
...
**什么是“当前读”和“快照读”？在RR隔离级别下，如何彻底解决幻读？**

-   **快照读**：普通的SELECT语句（不加锁），读取的是历史版本数据（基于Undo），利用MVCC实现，不阻塞写操作。
-   **当前读**：读取的是数据库当前最新的已提交版本。包括：`SELECT ... FOR UPDATE` / `LOCK IN SHARE MODE`、`UPDATE`、`DELETE`、`INSERT`。

**彻底解决幻读（RR级别）**：
快照读天然避免幻读（因为看到的是旧快照）。当前读通过 **Next-Key Lock**（Record Lock + Gap Lock）解决幻读。
例如 `SELECT * FROM t WHERE id > 10 FOR UPDATE`，会对索引上的间隙加锁，阻止其他事务在该间隙中 `INSERT` 新数据，从而保证当前读两次读到的记录集一致。
...
```sql
6-byte DB_TRX_ID 
## 最近更新本行记录的事务ID。 用来标识最近一次对本行记录做修改 (insert 、update) 的事务的标识符 ，即最后一次修改本行记录的事务 id。 如果是 delete 操作， 在InnoDB存储引擎内部也属于一次 update 操作，即更新行中的一个特殊位 ，将行标识为己删除，并非真正删除。

7-byte DB_ROLL_PTR
## 回滚指针，指向Undo Log中该行历史版本的指针。如果该行未被更新，则为空.

6-byte DB_ROW_ID
## 单调递增的行ID（如果表没有主键时使用）。如果没有设置主键且该表没有唯一非空索引时，InnoDB会使用该 id 来生成聚簇索引.
```

**可见性判断逻辑 (ReadView)**：

-   **m_ids**：当前活跃（未提交）的所有事务ID列表。
-   **min_trx_id**：m_ids中的最小值。
-   **max_trx_id**：下一个将要分配的事务ID。

判断规则（针对当前读的RR/RC隔离级别下的快照读）：

1.  若 `DB_TRX_ID` < `min_trx_id`：数据在该ReadView创建前已提交，**可见**。
2.  若 `DB_TRX_ID` >= `max_trx_id`：数据是ReadView创建后开启的事务修改的，**不可见**。
3.  若 `min_trx_id` <= `DB_TRX_ID` < `max_trx_id`：
    -   如果 `DB_TRX_ID` 在 `m_ids` 中，说明事务未提交，**不可见**，需通过 `DB_ROLL_PTR` 找上一个版本。
    -   如果不在 `m_ids` 中，说明事务已提交，**可见**。
...
==RC与RR的区别在于ReadView的生成时机==。RC是每次快照读都生成新的ReadView；RR是第一次快照读时生成，整个事务期间复用。
...
-   **快照读**：普通的SELECT语句（不加锁），**读取的是历史版本数据（基于Undo）**，利用MVCC实现，不阻塞写操作。可重复读（repeatable-read）下 mvcc生效读取的是数据的快照，并不是最新版本的数据（未提交事物的数据）
-   **当前读**：**读取的是数据库当前最新的已提交版本**。包括：`SELECT ... FOR UPDATE` / `LOCK IN SHARE MODE`、`UPDATE`、`DELETE`、`INSERT`。
...
**XA事务**：一种分布式事务规范（eXtended Architecture），通过两阶段提交（Prepare + Commit/Rollback）保证跨多个资源（如多个MySQL实例、MySQL+Redis）的原子性。
**MySQL支持**：InnoDB支持XA，语法为 `XA START`, `XA END`, `XA PREPARE`, `XA COMMIT`。MySQL内部的主从复制也利用了XA（Binlog与Redo Log的XA协调）。
...
| :-------------------------- | :----------------------------------------------------------- | :----------------------- | :----------------------------------------------------------- | :----------------------------------------------------------- |
| **1  实时写，实时刷(默认)** | 每次事务提交，都将 redo log 从 buffer **立即写入并持久化**到磁盘文件。 | 最低（约比`0`值低29%）   | **最高**。保证事务持久性（ACID的D），系统崩溃也不丢数据。    | 金融、订单等对数据一致性要求极高的核心系统。                 |
| 2：实时写，延时刷           | 每次事务提交，将 redo log 写入**操作系统缓存 (Page Cache)**，由操作系统决定何时真正写入磁盘（通常每秒一次）。 | 中等（约比`0`值低12.5%） | **较高**。MySQL崩溃不影响，但操作系统崩溃可能丢失**最近1秒内**的数据。 | 多数业务系统可接受的折中方案，平衡了性能与安全。             |
| 0：延迟写                   | 事务提交时**完全不触发**写盘操作，由后台线程**每隔1秒**统一将 buffer 中的日志写入并持久化。 | **最高**                 | **最低**。数据库崩溃时，可能丢失**最近1秒内**所有未刷盘的事务数据。 | 对数据一致性要求不高的场景，如测试环境、批量数据导入、日志系统等。 |
...
#### ✍️发散问题1、事务还没提交的时候，redo log 能不能被持久化到磁盘？

==redo log持久化==：缓冲区的数据一般情况下是无法直接写入磁盘中的，中间必须要经过OS的缓冲区，再通过系统调用fsync()将其刷到磁盘。

redo log 三种状态：1）存在于redolog buffer中 ；2）向磁盘写入，但是还没有真正落盘，而是保存在OD Buffer中； 3）持久化到磁盘中
...
### 117 什么是分布式事务中的XA事务？MySQL对XA的支持如何？测试XA有什么难点？

**测试难点**：

1.  **Prepare阶段后的宕机恢复**：事务在PREPARE状态后数据库宕机，重启后如何恢复？需要人工介入 `XA RECOVER` 查看悬空事务，决定提交或回滚。
...
### 120 线上需要删除1亿条历史数据，直接执行 `DELETE FROM big_table LIMIT 1000000` 放在一个事务里会有什么问题？应该如何改进？

**问题**：

1.  **锁膨胀**：一个事务删除百万行，会持有对应行及间隙的锁时间过长，阻塞业务读写。
2.  **Undo Log暴涨**：InnoDB需要保存这百万行的Undo信息以支持回滚，可能导致磁盘暴涨，甚至撑爆日志文件。
3.  **主从延迟**：主库执行完才会将百万行的变更一次性传给从库，造成从库长时间延迟。

**改进策略**：
**分批、小事务、低峰期**。

```sql
-- 建议每次删除几千行，事务提交，Sleep一下
DELETE FROM big_table WHERE create_time < '2020-01-01' LIMIT 1000;
COMMIT;
-- 通过脚本循环，或者使用 pt-archiver 工具
```

**工具推荐**：`pt-archiver`（Percona Toolkit），它能自动分批、控制延迟、将数据归档到另一张表或文件。
